...
## 15. 手机端（2026-09-19，用户要求：手机上看这个界面）

**结论**：手机上不再是一个「缩小的桌面」——左栏搬到屏幕底部当标签栏，顶栏压成一行，
表格变卡片，点按目标 ≥ 34px，输入框 16px（iOS 上小于 16px 会自动放大页面）。

- 断点只有一个：**≤720px**（规则分别写在 `src/style.css` 的一处总则和各组件自己的 `@media` 里，
  桌面一个像素都不受影响）；
- 顶栏：品牌 / 页面名 / 演示开关 / 两个状态胶囊挤进一行，胶囊和开关在窄屏换成短文案
  （`演示`、`小红书`、`0 项`），不再折成竖排；
- 左栏：`NavRail.vue` 在手机上 `flex-direction: row` + `order: 2`，变成底部标签栏，
  图标 19px、标签 10px；「有任务在跑」那个小圆点保留在最右侧；
- 工作台：对话占满整屏（右栏在 ≤1180px 就已收起），输入区收窄、提示左对齐、
  **「开始」占满一整行**（它是主行动），窄屏换一句更短的占位文案；
- 运行记录：六列表格在 390px 上只能横着拖 → 每行变一张卡片（标题/来源、状态 + 查看、六段进度条、
  产物/耗时/提交），表头隐藏，格子的名字用 `::before` 补上；
- 作品库：封面墙的列宽是行内样式按平台比例算的，手机上用 `!important` 压回单列；
- 风格页 / 设置页：技能行与配置行都改成「说明一行、控件一行」，不溢出。

**手机怎么访问**（原来打不开的真实原因有两条，都在服务端）：

1. vite dev server 绑的 `127.0.0.1` → 手机连不上（`ops/start_all.sh` 改成 `--host 0.0.0.0`，
   `vite.config.ts` 里也标了 `host: '0.0.0.0'`）；
2. 接口地址写死 `http://127.0.0.1:8000` → 在手机上那指的是**手机自己**。
   `vite.config.ts` 现在把 `/api`、`/artifacts` 代理到本机后端，前端在「页面不是本机」时自动改用
   同源相对地址（`src/api/index.ts` 的 `resolveBase`）—— 不暴露后端、也没有 CORS 问题。

手机上打开：**http://<这台机器的局域网 IP>:5178/**（dev server 启动日志里会直接打印 Network 那一行）。
注意：局域网内谁都能打开这个前端（也就等于能用这台机器上的模型额度）；不想暴露就把
`ops/start_all.sh` 里那行改回 `--host 127.0.0.1`。

**核验**：`node ops/shot/mobile_check.mjs [origin] [宽] [高]`（默认 390×844）——
七个页面逐页量「横向溢出 / 越界元素 / 小于 32px 的按钮」，再展开产物查看器量一遍，最后打印控制台错误数；
截图落 `docs/evidence/mobile-*.png`。实测 360×780、390×844、430×932 三种尺寸：
**七页 + 查看器全部 0 溢出、0 控制台错误**，手机尺寸下走局域网地址（Host 是 IP）结果与 127.0.0.1 一致。

> 两个只在这台机器上才遇到的坑（已写进技能）：
> ① `ops/start_all.sh` 里的 `--host 127.0.0.1` 会**盖掉** `vite.config.ts` 的 host，改配置不生效时先看这行；
