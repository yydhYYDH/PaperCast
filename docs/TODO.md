...
| 2026-09-19 | 前端 agent（用户点名三条） | **A25 点发布别再等：探登录态一次 HTTP、两把锁、8 张图一次传完**。① `_login_state` 原来起**两次**浏览器（check_login_status + /api/v4/me），改成带 cookies 打一次 `https://www.zhihu.com/api/v4/me`（向知乎本人接口核实）→ **8.0s → 0.22s**，后端 `/api/platforms?force=1` **32s → 0.24s**；HTTP 问不上才退浏览器兜底，返回值 `checkedBy` 如实标注 api/browser/cookies。**坑**：本服务解释器是 `var/toolchains/zhihu-mcp-venv`（没有 httpx），第一版 httpx 直接 500（我一度把 500 的耗时当成 0.019s 的成功），改用标准库 urllib 才对；已写进 README。② 拆锁：`_PUBLISH_LOCK` 只给真发布，`_BROWSER_LOCK` 给运营数据/核验/兜底探测 —— 三个探测接口并发从 61.7/54.1/45.8s 降到 **1.0/0.002/1.1s**。③ 弹窗 input 支持 multiple 时一次 `set_input_files([...8 张])`，轮询 1s→0.25s、12s 无变化即判卡住；不支持才回退逐张；差额只报不重传。dry_run 实测 **61.1s → 24.0s**、8/8 张进正文、正文块数 136 不变。**真界面实测**：面板 97ms、面板到「确认发布」可点 **288ms**（改前 17365ms）。核验回放三条不变，后端 pytest 351 passed。 |
