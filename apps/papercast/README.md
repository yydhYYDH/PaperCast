...
src/
  ...
  views/          运行历史、作品库、平台账号、运营维护、引擎与环境
  data/works.ts   作品库的「一件作品」推导：封面 / 正文 / 成片 / 发布状态，全从真产物与发布回执来
...
左侧「作品库」回答一个问题：**这些稿子做出来了没有、发出去了没有**。

一件作品 = 一次运行里、面向一个平台的那份稿子（推导集中在 `src/data/works.ts`，视图里不再散落判断）：

- 卡片上摆的是**真封面**：小红书用它的第一张卡片（1080×1440，3:4）、知乎用横版配图（1600×1200）、B 站用成片封面（1920×1080），格子按各平台自己的比例开框，一眼能分出竖版图文和横屏视频；
- 卡片右下角是**发布状态**，来自后端 `publish/receipts.json` 与发布阶段的检查项：已发布（带链接）／存了草稿／渠道没就绪（附「MCP 在线但未登录」这类原话）／没跑完／还没发。**没有回执就如实说阶段走到哪一步，不编数字**；
- 点开就是这一件本身：一篇文章（渲染后的正文，小红书还会把 6 张真卡片横着摆出来）或一条视频（真播放器）；右边窄栏给状态、来源论文、以及动作（打开原稿／成片、复制封面路径、在运行里打开），同一批的其它件可以一键切换；
- 每个平台先摆最新 6 件，更早的折成一行列表 —— 这台机器跑过 20 多次，全铺开来只是同一张封面重复 40 遍。

核验：`node ops/shot/library_check.mjs`（封面是否真的解码出来、状态是否有回执依据、点开是不是文章/视频、窄屏是否溢出），证据落 `docs/evidence/library-*.png`。
