      ...
      <div class="pub" role="dialog" aria-modal="true" aria-label="发布这件作品">
        ...
        <div class="pub-body">
          ...
          <div v-if="pub.loading" class="pub-note">
            正在看这次运行产出了什么、各平台现在能不能收…（要真去问一次渠道状态：正常一秒内就有结果；
            网络不通的时候会退到「起一个浏览器去确认」，那才要几秒到几十秒）
          </div>
              ...
              <div class="gate" :class="pub.canPublish ? 'ok' : 'blocked'">
                <template v-if="pub.canPublish">
                  ...
                  <div v-if="pub.busy === 'publish' || pub.busy === 'video'" class="waiting">
                    ...
                    <p class="wait-hint">
                      那边现在在做：打开浏览器 → 填标题正文 → 传图 → 点发布 → 回来核验这一篇是不是真的发出去了。
                      图文一般要几十秒（本机实测知乎一篇：填稿+传 8 张图 24 秒，再加点发布与核验）。别重复点 ——
                      关掉这个窗口不会取消，只是你看不到结果。
                    </p>
                  </div>
                </template>
